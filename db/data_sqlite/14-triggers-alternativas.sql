...
FOR EACH ROW BEGIN
	...
	UPDATE alternativas SET pobl_futura = ((SELECT CEIL(hab_conectados * (1 + (COALESCE(tasa_crecimiento, 0) * COALESCE(ano_horiz_sistema, 0) / 100))) FROM preferencias_disenho a JOIN alternativas b ON a.cod_alternativa = b.cod_alternativa WHERE b.gid = alternativas.gid) / 86400) WHERE gid = NEW.gid;

	UPDATE alternativas SET dem_poblacion = (SELECT COALESCE(f_var_estacional, 0) FROM preferencias_disenho WHERE cod_alternativa = alternativas.cod_alternativa) * (SELECT COALESCE(dot_cantareras, 0) FROM preferencias_disenho WHERE cod_alternativa = alt_conexiones.cod_alternativa) * pobl_futura WHERE gid = NEW.gid AND tipo_distribucion = 'Cantareras';
	UPDATE alternativas SET dem_poblacion = (SELECT COALESCE(f_var_estacional, 0) FROM preferencias_disenho WHERE cod_alternativa = alternativas.cod_alternativa) * (SELECT COALESCE(dot_domiciliar, 0) FROM preferencias_disenho WHERE cod_alternativa = alt_conexiones.cod_alternativa) * pobl_futura WHERE gid = NEW.gid AND tipo_distribucion = 'Domiciliar';
